...
export const getServerSideProps: GetServerSideProps = async ({query, res}) => {
    ...
    const [anime]: ISearch<IAnimeResult>[] = await Promise.all([AniList.getAdvancedSearch(query.q ? query.q as string : undefined, query.type ? query.type as string : undefined, query.page ? +query.page : undefined, query.season ? query.season as string : undefined, query.format ? query.format as string : undefined, query.sortBy ? (query.sortBy as string).split('$') : undefined, query.genres ? (query.genres as string).split('$') : undefined, query.id ? +query.id : undefined, query.year ? +query.year : undefined, query.status ? query.status as string : undefined)]);

    return {
        props: {
            anime: anime.results || null,
        }
    }
}
