...
const Carrousel = dynamic(() => import('@components/Anime/Carrousel'), {
    ssr: false,
});

const Featured = dynamic(() => import('@components/Anime/Featured'), {
    ssr: false,
});
...
export const getServerSideProps: GetServerSideProps = async ({res}) => {
    ...
    const featured: any = Object.values(trending.results).slice(0, 15);
    const rows: Array<object> = [
        ...
        {
            title: 'Trending',
            animes: JSON.parse(JSON.stringify(trending)).results
        },
        {
            title: 'Popular',
            animes: JSON.parse(JSON.stringify(popular)).results
        }
    ]
    return {
        props: {
            featured: JSON.parse(JSON.stringify(featured)) || null,
            rows: rows || null
        }
    }
}
